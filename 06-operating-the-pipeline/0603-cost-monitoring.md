...
**Storage costs** are predictable but sneaky at scale. Append-only tables and snapshot tables grow with every run, and without a retention policy that growth is unbounded ([[02-full-replace-patterns/0202-snapshot-append|0202]] covers retention and tiered compaction). Staging tables that outlive their load job are dead weight, though in practice orphaned staging is more of a schema hygiene issue than a cost problem -- at ~$0.02/GB/month in BigQuery, a few hundred GB of staging is annoying but not alarming. The compute cost of accidentally querying unpartitioned staging is usually worse than storing it.
