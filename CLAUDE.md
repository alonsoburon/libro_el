...
### Voice & Tone
- **Technical but human.** Write like a senior engineer explaining to a peer over coffee, not like a textbook. But don't overdo the personality -- no comedy, no "you know this table" filler. Direct and confident without performing. Don't get combative about uncontested claims either -- no "whether you admit it or not" on something nobody actually disputes; state it plainly.
...
- **No filler.** If a sentence doesn't teach something, cut it. This includes self-explaining restatements -- parenthetical asides or "regardless of whether X or Y" clauses that just re-say what the sentence already said. If removing a clause loses no information, remove it.
...
- **No negative parallelism.** "It's not A, it's B" is a dead giveaway of AI writing; so are "not only X, but also Y" and "not just A, it's B". Don't negate then correct -- state the thing directly. This includes the dash-interjection form "-- it changes X, never Y"; rewrite it as a relative clause ("which changes X without altering Y"). (Mild contrastive "X, not Y" where both sides are real distinct things is fine -- e.g. "a correctness parameter, not a performance parameter".)
...
- **No significance inflation.** Don't tell the reader something matters with empty intensifiers -- "this marks a pivotal moment", "underscores the importance of", "plays a crucial role", "is a testament to". The ban is on *empty* phrasing only: concrete or scope-quantifying tails ("over the life of the table") and a touch of evocative color ("at the worst possible moment") are fine and welcome. Likewise, present participles are good when they genuinely define or add ("compute a hash, fingerprinting the row's current state"); only cut significance-tail participles that pad ("...reducing cost, further improving reliability and strengthening the pipeline").
- **No staccato sequences.** Avoid chains of short declarative sentences ("X does this. Y does that. Z fails."). Combine related ideas into longer, flowing sentences using conjunctions, relative clauses, or dashes, since one well-constructed sentence beats three choppy ones. The "A happened, B didn't" / "A changed; B didn't" construction is a specific variant of this -- merge the two halves into one sentence that explains the relationship. Two exceptions the author keeps: (1) **Parallel anaphora that carries distinct content per item** -- "A field appears because the team shipped a feature... A field disappears because someone removed it from the API... A field that was always a string is now a number" stays, because each clause adds a different fact. But anaphora used as rhythmic padding gets collapsed: the author rewrote "Every date-based partition, every month-end report, every time-series chart is silently wrong" to "everything becomes silently wrong", and "No cursor state to maintain, no missed deletes, no drift accumulation" to "avoid cursor states, hard-deletions and drift". Test: if every item in the triple says something the others don't, keep it; if they're three ways of saying "all of it", collapse. (2) **Topic-sentence house style** -- a short declarative leading a paragraph followed by detail ("DML concurrency is the constraint. ...") is good structure, not staccato. When you do merge a chain, prefer stating the underlying principle and its reason over just enumerating the effects (e.g. "the job fails and stops, failing fast to avoid overwriting partitions with NULLs", not a four-sentence list of what stops).
