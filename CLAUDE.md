...
- **No staccato sequences.** Avoid chains of short declarative sentences ("X does this. Y does that. Z fails."). Combine related ideas into longer, flowing sentences using conjunctions, relative clauses, or dashes, since one well-constructed sentence beats three choppy ones. The "A happened, B didn't" / "A changed; B didn't" construction is a specific variant of this -- merge the two halves into one sentence that explains the relationship. Two exceptions the author keeps: (1) **deliberate parallel anaphora** -- repeated identical structure for rhetorical effect ("A field appears because... A field disappears because... A field that was always a string is now...") is intentional, not choppy; leave it. (2) **Topic-sentence house style** -- a short declarative leading a paragraph followed by detail ("DML concurrency is the constraint. ...") is good structure, not staccato. When you do merge a chain, prefer stating the underlying principle and its reason over just enumerating the effects (e.g. "the job fails and stops, failing fast to avoid overwriting partitions with NULLs", not a four-sentence list of what stops).
